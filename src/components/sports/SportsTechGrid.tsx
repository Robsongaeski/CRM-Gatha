import { Wind, Scissors, Palette, ShieldCheck, type LucideIcon } from "lucide-react";
import { sportTech } from "@/data/sports";
import { ScrollReveal } from "@/components/category/ScrollReveal";

const iconMap: Record<string, LucideIcon> = {
  wind: Wind,
  "shield-sun": ShieldCheck,
  scissors: Scissors,
  palette: Palette,
};

export function SportsTechGrid() {
  return (
    <section className="bg-background py-16 lg:py-24" aria-labelledby="sports-tech-heading">
      <div className="max-w-6xl mx-auto px-5 lg:px-8">
        <ScrollReveal className="text-center max-w-2xl mx-auto mb-10 lg:mb-14">
          ...
          <h2
            id="sports-tech-heading"
            className="mt-3 text-3xl lg:text-5xl font-black tracking-tight text-foreground"
          >
            Tecnologia que joga<br className="hidden sm:block" /> com você.
          </h2>
          <p className="mt-4 text-sm lg:text-base text-muted-foreground">
            Cada detalhe pensado para resistir ao jogo, ao treino e à temporada inteira.
          </p>
        </ScrollReveal>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-5">
          {sportTech.map((feature, idx) => {

...
export default function Atendimento() {
  ...
  const handleInstanceSelected = useCallback(async (
    ...
    try {
      ...
      // Usar refetchQueries para aguardar o refetch completar
      ...
      if (conv) {
        const contactPhoneValue = conv.contact_phone || conv.remote_jid?.replace('@s.whatsapp.net', '') || '';
        const groupKey = contactPhoneValue.slice(-8);
        
        const tempGroup: GroupedConversation = {
          groupKey,
          mainName: conv.contact_name || contactPhoneValue,
          photoUrl: conv.contact_photo_url || null,
          isGroup: conv.is_group,
          conversations: [conv],
          lastMessageAt: conv.last_message_at || new Date().toISOString(),
  ...
  }, [queryClient, connectedInstances]);

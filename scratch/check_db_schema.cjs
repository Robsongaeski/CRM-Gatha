...
async function checkSchema() {
  ...
  console.log('--- Conversas com Retorno Pendente ---');
  ...
  if (conversations.length > 0) {
    const clienteIds = conversations.map(c => c.cliente_id).filter(Boolean);
    if (clienteIds.length > 0) {
      console.log('--- Leads vinculados a essas conversas ---');
      const leadsQuery = `
        SELECT id, cliente_id, data_retorno, status 
        FROM leads 
        WHERE cliente_id IN (${clienteIds.map(id => `'${id}'`).join(',')})
      `;
      const leads = await runQuery(leadsQuery);
